  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-end justify-between gap-3">
        <div>
          <h3 className="text-sm font-extrabold text-slate-900">{title}</h3>
          <p className="mt-1 text-xs text-slate-600">全体と比べて特徴が出ている上位3つ</p>
        </div>
      </div>
...
export default function AdminCompare() {
  ...
  const hasSegmentFilter = Boolean(ageBand || gender);
  ...
  const topInsights = useMemo(() => {
    if (!segSummaryPack?.diffs || !hasSegmentFilter) return [];

    const items = (segSummaryPack.diffs[groupKey] ?? []) as ApiDiffItem[];

    return [...items]
      .sort((x, y) => Math.abs(y.diffPt) - Math.abs(x.diffPt))
      .slice(0, 3);
  }, [segSummaryPack, groupKey, hasSegmentFilter]);
  ...
  const periodLeftLabel =
    basis === "baseline" ? "期間Bの回答割合（全体）" : "期間Bの回答割合（指定した属性）";
  const periodRightLabel =
    basis === "baseline" ? "期間Aの回答割合（全体）" : "期間Aの回答割合（指定した属性）";
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-10">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <Kpi label="期間Aの回答数（全体）" value={data?.periodA?.baselineTotal ?? "-"} />
          <Kpi label="期間Aの回答数（指定した属性）" value={data?.periodA?.segmentTotal ?? "-"} />
          <Kpi label="期間Bの回答数（全体）" value={data?.periodB?.baselineTotal ?? "-"} />
          <Kpi label="期間Bの回答数（指定した属性）" value={data?.periodB?.segmentTotal ?? "-"} />
        </section>
        ...
        <Block
          step="1"
          title="何を知りたいか"
          description="どの項目について結果を見るかを選びます"
        >
        ...
        <Block
          step="2"
          title="誰の回答を見るか"
          description="深掘りしたい来館者属性を指定します"
        >
          <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
            <div className="mt-1 text-[11px] text-slate-500">
              属性を指定しない場合は全体の傾向を表示します
            </div>
        ...
        >
          <div className="space-y-5">
            <div className="flex flex-wrap items-center gap-3">
              <div className="text-xs font-extrabold text-slate-700">期間入力方法</div>
            ...
            <div className="grid gap-3 md:grid-cols-12">
              <div className="md:col-span-6 rounded-2xl border border-slate-100 bg-slate-50 p-4">
                <div className="text-xs font-extrabold text-slate-700">期間A</div>
              ...
              <div className="md:col-span-6 rounded-2xl border border-slate-100 bg-slate-50 p-4">
                <div className="text-xs font-extrabold text-slate-700">期間B</div>
                <div className="mt-2 grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-xs font-bold text-slate-700">from</label>
                    <input
        ...
        <Block
          step="4"
          title="どう比較するか"
          description="全体との差を見るか、期間どうしを比べるかを選びます"
        >
          <div className="space-y-4">
            ...
            {analysisMode === "seg" ? (
              <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                <div className="text-xs font-extrabold text-slate-700">特徴を見る対象期間</div>
                <div className="mt-2 flex flex-wrap gap-2">
                  <button
                    className={`rounded-xl px-3 py-1.5 text-sm font-bold border ${
                      segPeriod === "A"
              ...
              <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                <div className="text-xs font-extrabold text-slate-700">比較対象</div>
                <p className="mt-1 text-[11px] text-slate-500">
                  全体同士で比較するか、指定した属性同士で比較するかを選びます
                </p>
                <div className="mt-2 flex flex-wrap gap-2">
                  <button
                    className={`rounded-xl px-3 py-1.5 text-sm font-bold border ${
        ...
        <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-5 text-sm text-slate-700">
            ...
            <div>
              ...
              {analysisMode === "seg"
                ? `全体比較（${segPeriod === "A" ? "期間A" : "期間B"}）`
                : `期間比較（${basis === "baseline" ? "全体" : "指定した属性"}）`}
            </div>
          </div>
        </section>
        ...
        <section className="space-y-6">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div>
              <h2 className="text-sm font-extrabold text-slate-900">基本分析</h2>
              <p className="mt-1 text-xs text-slate-600">
                属性指定の有無にかかわらず、全体の内訳や期間比較を確認できます
              </p>
            </div>
            ...
            <div className="mt-4">
              {analysisMode === "seg" ? (
                <DiffTable
                  ...
                  rows={segRows}
                  leftLabel="全体の回答割合"
                  rightLabel="指定した属性の回答割合"
                  rightCountLabel="属性人数"
                />
              ) : (
                <DiffTable
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div>
              <h2 className="text-sm font-extrabold text-slate-900">深掘り分析</h2>
              <p className="mt-1 text-xs text-slate-600">
                年代・性別などを指定すると、全体との差が大きい項目を確認できます
              </p>
            </div>
            ...
            {!hasSegmentFilter ? (
              <div className="mt-4 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-6">
                <div className="text-sm font-bold text-slate-700">
                  属性を指定すると、深掘り分析を表示できます
                </div>
                <p className="mt-2 text-sm text-slate-600">
                  たとえば「30代」「女性」を指定すると、全体と比べて差が大きい項目や、設問ごとの特徴を確認できます。
                </p>
              </div>
            ) : analysisMode !== "seg" ? (
              <div className="mt-4 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-6">
                <div className="text-sm font-bold text-slate-700">
                  深掘り分析は「全体と比べて特徴を見る」で表示されます
                </div>
                <p className="mt-2 text-sm text-slate-600">
                  属性を指定した状態で、「全体と比べて特徴を見る」を選ぶと、特徴サマリーと詳しい結果を表示できます。
                </p>
              </div>
            ) : (
                ...
                <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex flex-col gap-4">
                    <div>
                      ...
                      <div className="mt-1 text-xs text-slate-600">
                        {segPeriod === "A" ? "期間A" : "期間B"} の「{ageBand || ""}{gender || ""}」について、
                        {GROUP_TITLES[groupKey]}の中で全体との差が大きい項目です
                      </div>
                    </div>
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              {currentWallpaperPath && (
                <p className="mt-2 text-xs text-slate-600">
                  現在の壁紙ファイル: <span className="font-mono">{currentWallpaperPath}</span>
                </p>
              )}
            </div>
          </div>
          ...
          <div className="mt-4 grid gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="mt-2 text-xs text-slate-500">推奨: スマホ縦長JPEG。10MBまで。</p>
              </div>
